...
#include "../serial_stream/lora_stream.h"
...
const ResultCode RESULT_CODES[] = {
    ...
    { AtResponse::PARAM_ERROR, "PARAM_ERROR", 11 }
};
...
int AtParserImpl::readMore(unsigned* timeout) {
    ...
    for (;;) {
        ...
        auto t = millis();
        CHECK(strm->waitEvent(LoraStream::READABLE, waitTimeout));
        if (timeout) {
            t = millis() - t;
            if (t >= *timeout) {
    ...
    *size = 0;
    for (;;) {
        ...
        *size += n;
        ...
        CHECK(strm->waitEvent(LoraStream::WRITABLE, waitTimeout));
        if (timeout) {
...
void AtParserImpl::logCmdLine(const char* data, size_t size) const {
    if (size > 0) {
        ...
        LOG_PRINTF_C(TRACE, conf_.logCategory(), "%010lu [%s] TRACE: > %.*s\r\n", millis(), conf_.logCategory(), size, data);
    }
}
