...
void setupStm32();
int performFlashSteps(ApplicationAsset& asset, uint32_t options);
int enterBootloader(uint32_t options);
void resetStm32(uint32_t options);
void exitBootloader(uint32_t options);
int getInfo();
int eraseFlash();
...
static std::pair<uint8_t, uint8_t> bootPin_;
static std::pair<uint8_t, uint8_t> resetPin_;

int flashStm32Binary(ApplicationAsset& asset, std::pair<uint8_t, uint8_t> boot0Pin, std::pair<uint8_t, uint8_t> resetPin, uint32_t options) {
  bootPin_ = boot0Pin;
  resetPin_ = resetPin;

  setupStm32();
  int ret = performFlashSteps(asset, options);
  exitBootloader(options);
  return ret;
}
...
int enterBootloader( uint32_t options) {
  LOG(INFO, "Resetting STM32 into bootloader mode, options: %x", options);

  // Set BOOT0 pin HIGH to enter the bootloader mode
  Mcp23s17::getInstance().writePinValue(bootPin_.first, bootPin_.second, (options & STM32_BOOT_NONINVERTED) ? HIGH : LOW);
...
void resetStm32(uint32_t options) {
  Mcp23s17::getInstance().writePinValue(resetPin_.first, resetPin_.second, (options & STM32_RESET_NONINVERTED) ? LOW : HIGH);
  delay(200);
  Mcp23s17::getInstance().writePinValue(resetPin_.first, resetPin_.second,(options & STM32_RESET_NONINVERTED) ? HIGH : LOW);
  delay(200);
}

void exitBootloader(uint32_t options) {
  // Set BOOT0 pin LOW to run the program
  Mcp23s17::getInstance().writePinValue(bootPin_.first, bootPin_.second, (options & STM32_BOOT_NONINVERTED) ? LOW : HIGH);
  delay(1);
