...
int Mcp23s17::reset(bool verify) {
    ...
    if (IOE_RST != PIN_INVALID) {
        ...
        CHECK(writeContinuousRegisters(0x00, DEFAULT_REGS_VALUE, sizeof(DEFAULT_REGS_VALUE)));
    }
    ...
    if (verify) {
        LOG(INFO, "Verifying IO Expander registers after reset...");
        bool success = false;
        uint8_t tmp[22] = {0x00};
        SCOPE_GUARD ({
            if (!success) {
                LOG(ERROR, "IO expander verification failed!");
                for (uint8_t i = 0; i < sizeof(tmp); i++) {
                    Log.printf("0x%02x ", tmp[i]);
                }
                Log.print("\r\n");
                RGB.control(true);
                RGB.color(0xFF, 0, 0);
        ...
        success = true;
        LOG(INFO, "IO expander verification passed!");
    }
    resetRegValue();
    return SYSTEM_ERROR_NONE;
}
...
void Mcp23s17::resetRegValue() {
    // FIXME: IOCON value depends on HAL_PLATFORM_MCP23S17_MIRROR_INTERRUPTS and HAL_PLATFORM_MCP23S17_OPEN_DRAIN_INT
...
int Mcp23s17::writeContinuousRegisters(const uint8_t start_addr, const uint8_t* val, uint8_t len) const {
    CHECK_TRUE(initialized_, SYSTEM_ERROR_NONE);
